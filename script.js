...
function handleJump() {
  if (player.isJumping) {
    ...
    if (player.y < canvas.height - player.height - 65) {
      player.y += player.speed;
    }
  }
}
...
function handleKeyDown(event) {
  if (event.keyCode === 32) {
    ...
    // Left arrow key
    const positionAfterMoving = player.x - player.speed;
    if(positionAfterMoving > 0) player.x = positionAfterMoving;
  } else if (event.keyCode === 39) {
    // Right arrow key
    const positionAfterMoving = player.x + player.speed;
    if(positionAfterMoving < 700) player.x = positionAfterMoving;
  }
}
